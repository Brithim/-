(function() {
    ...
    function draw() {
        ...
        px = mouseX;
        py = mouseY;
        points.push(new Point(px, py, dx, dy, Math.abs(Math.sin(size += 0.155) * 10) + 1, 'rgb(' +
            (Math.floor(Math.sin(red += RED_STEP) * 128 + 200)) + ',' +
            (Math.floor(Math.sin(green += GREEN_STEP) * 0 + 0)) + ',' +
            (Math.floor(Math.sin(blue += BLUE_STEP) * 0 + 0)) + ')'));
